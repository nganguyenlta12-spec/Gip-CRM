...
const SOURCES = ["Tự tìm", "Referral", "Inbound", "Event", "Social Media"];
...
export default function App() {
  ...
  const [tab, setTab] = useState("pipeline"); // "pipeline" | "report"
  ...
  // Init reportMonth to current month
  useEffect(() => {
  ...
  const saveDeal = (deal) => {
    ...
    if (deal.id) {
      setDeals(p => p.map(d => {
        ...
        // if stage changed, append history
        let history = Array.isArray(d.stageHistory) ? [...d.stageHistory] : [];
        if (d.stage !== deal.stage) {
          history.push({ from: d.stage, to: deal.stage, date: now });
        }
      ...
      const newDeal = {
        ...
        stage: deal.stage || "Freeze",
        createdAt: now,
        updatedAt: now,
        stageHistory: [{ from: null, to: deal.stage || "Freeze", date: now }],
      };
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#f0f4f8", minHeight: "100vh", color: "#1a2a3a" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #dde6f0", padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px", boxShadow: "0 1px 4px rgba(0,80,160,0.07)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: "17px", color: "#1a6fba", lineHeight: 1.1 }}>Sales Pipeline PKD2 CRM</div>
            <div style={{ fontSize: "9px", color: "#90a8c0", letterSpacing: "0.08em", marginTop: "1px" }}>GIP FULFILLMENT</div>
          </div>
        </div>
      ...
      {STAGES.map(stage => {
        ...
        return (
          <div key={stage}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: "10px", borderBottom: `1.5px solid ${cfg.border}` }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                {stageDeals.some(d => d.value) && (
                  <span style={{ fontSize: "10px", color: "#b86e00", fontWeight: "600" }}>
                    {(stageDeals.reduce((s, d) => s + (Number(d.value) || 0), 0) / 1e6).toFixed(0)}M₫
                  </span>
                )}
            ...
            {stageDeals.length === 0 && (
              <div style={{ padding: "20px 0", textAlign: "center", color: isOver ? cfg.color : "#c0cfd8", fontSize: "11px" }}>
                {isOver ? "↓ Thả vào đây" : "Chưa có deal"}
              </div>
            )}
    ...
    <div draggable onDragStart={onDragStart} onDragEnd={onDragEnd}
      ...
      {platforms.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginTop: "7px" }}>
          {platforms.map(p => (
            <span key={p} style={{ background: cfg.badge, border: `1px solid ${cfg.border}`, borderRadius: "4px", padding: "1px 6px", fontSize: "10px", color: cfg.color, fontWeight: "500" }}>{p}</span>
          ))}
        </div>
      )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "8px", paddingTop: "7px", borderTop: "1px solid #eef3f8" }}>
        ...
        {history.length > 1 && (
          <button onClick={e => { e.stopPropagation(); setShowHistory(v => !v); }}
            style={{ background: "transparent", border: "none", fontSize: "10px", color: "#1a6fba", cursor: "pointer", fontFamily: "inherit", padding: 0 }}>
      ...
      {showHistory && history.length > 0 && (
        <div style={{ marginTop: "8px", background: "#f4f8fc", borderRadius: "7px", padding: "8px", display: "flex", flexDirection: "column", gap: "4px" }}>
          {history.filter(h => h.from).map((h, i) => {
            const prev = history[i] || null;
            const days = prev ? daysBetween(history[i-1]?.date || deal.createdAt, h.date) : 0;
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "5px", fontSize: "10px" }}>
                <span style={{ color: fromCfg.color || "#90a8c0", fontWeight: "600" }}>{fromCfg.icon} {h.from}</span>
                <span style={{ color: "#c0cfd8" }}>→</span>
                <span style={{ color: toCfg.color || "#1a7a45", fontWeight: "600" }}>{toCfg.icon} {h.to}</span>
                <span style={{ color: "#a0b8d0", marginLeft: "auto" }}>{days}d · {fmtDate(h.date)}</span>
              </div>
            );
          })}
        </div>
...
function ReportView({ deals, allMonths, reportMonth, setReportMonth }) {
  ...
  // Deals that moved to a stage this month
  const movedThisMonth = deals.flatMap(d => {
    const hist = Array.isArray(d.stageHistory) ? d.stageHistory : [];
    return hist.filter(h => h.from && monthKey(h.date) === reportMonth).map(h => ({ ...h, brand: d.brand, dealId: d.id }));
  });

  // Win this month
  const wonThisMonth = deals.filter(d => {
    const hist = Array.isArray(d.stageHistory) ? d.stageHistory : [];
    return hist.some(h => h.to === "Win" && monthKey(h.date) === reportMonth);
  });

  // Avg days per stage transition overall
  const avgDays = {};
  STAGES.forEach((st, i) => {
    if (i === 0) return;
    ...
    const transitions = deals.flatMap(d => {
      const hist = Array.isArray(d.stageHistory) ? d.stageHistory : [];
      return hist.filter(h => h.from === fromStage && h.to === st).map((h, idx) => {
        const prevDate = hist.slice(0, hist.indexOf(h)).reverse().find(x => x.to === fromStage)?.date || d.createdAt;
        return daysBetween(prevDate, h.date);
      });
    });
    avgDays[`${fromStage}→${st}`] = transitions.length ? Math.round(transitions.reduce((a, b) => a + b, 0) / transitions.length) : null;
  });

  // Per-stage count this month
  const stageCounts = {};
    ...
    <div style={{ padding: "20px", maxWidth: "900px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #dde6f0", borderRadius: "12px", padding: "16px", boxShadow: "0 1px 4px rgba(0,80,160,0.07)" }}>
          <div style={{ fontWeight: "700", color: "#1a2a3a", fontSize: "13px", marginBottom: "14px" }}>📋 Phân bổ theo stage (leads mới tháng này)</div>
          {STAGES.map(st => {
        ...
        {/* Avg transition days */}
        <div style={{ background: "#fff", border: "1px solid #dde6f0", borderRadius: "12px", padding: "16px", boxShadow: "0 1px 4px rgba(0,80,160,0.07)" }}>
          <div style={{ fontWeight: "700", color: "#1a2a3a", fontSize: "13px", marginBottom: "14px" }}>⏱ Avg ngày chuyển stage (toàn bộ deals)</div>
          {Object.entries(avgDays).map(([key, val]) => {
            ...
            return (
              <div key={key} style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px", background: "#f4f8fc", borderRadius: "8px", padding: "8px 10px" }}>
                ...
                <span style={{ marginLeft: "auto", fontSize: "13px", fontWeight: "700", color: val !== null ? "#1a6fba" : "#c0cfd8" }}>
                  {val !== null ? `${val} ngày` : "—"}
                </span>
              </div>
            );
      ...
      <div style={{ width: "480px", maxWidth: "90vw" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "13px" }}>
          ...
          <Field label="Giai đoạn Pipeline" span>
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
              {STAGES.map(st => {
                ...
                return (
                  <button key={st} onClick={() => s("stage", st)}
                    style={{ background: active ? cfg.badge : "#f4f8fc", border: `1.5px solid ${active ? cfg.border : "#dde6f0"}`, borderRadius: "8px", padding: "5px 12px", color: active ? cfg.color : "#90a8c0", fontSize: "12px", fontWeight: active ? "700" : "400", cursor: "pointer", fontFamily: "inherit", transition: "all 0.12s" }}>
                    {cfg.icon} {st}
                  </button>
                );
              })}
              ...
              {PLATFORMS.map(p => {
                ...
                return (
                  <button key={p} onClick={() => togglePlatform(p)}
                    style={{ background: active ? "#e8f3fc" : "#f4f8fc", border: `1.5px solid ${active ? "#90c0ef" : "#dde6f0"}`, borderRadius: "8px", padding: "5px 12px", color: active ? "#1a6fba" : "#90a8c0", fontSize: "12px", fontWeight: active ? "600" : "400", cursor: "pointer", fontFamily: "inherit" }}>
                    {p}
                  </button>
                );
              })}
          ...
          <Field label="Nguồn lead">
            <select value={f.source} onChange={e => s("source", e.target.value)}
              style={{ background: "#f4f8fc", border: "1px solid #c8ddf0", borderRadius: "8px", padding: "8px 10px", color: f.source ? "#1a2a3a" : "#90a8c0", fontSize: "13px", width: "100%", outline: "none", fontFamily: "inherit" }}>
        ...
        {!isNew && f.createdAt && (
          <div style={{ fontSize: "11px", color: "#a0b8d0", marginTop: "12px" }}>
            📅 Ngày tạo: {fmtDate(f.createdAt)} · Cập nhật: {fmtDate(f.updatedAt)}
          </div>
        )}

        <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "16px" }}>
          <Btn onClick={onClose}>Huỷ</Btn>
          <Btn blue onClick={() => { if (!f.brand.trim()) return alert("Vui lòng nhập tên Brand!"); onSave(f); }}>
            {isNew ? "Tạo Deal" : "Lưu thay đổi"}
          </Btn>
        </div>
      </div>
    </Modal>
    ...
    <Modal onClose={onClose}>
      <div style={{ width: "540px", maxWidth: "92vw" }}>
        ...
        <div style={{ background: "#f0f7ff", border: "1px solid #c8ddf0", borderRadius: "10px", padding: "14px", marginBottom: "14px" }}>
          <div style={{ fontSize: "11px", color: "#1a6fba", fontWeight: "600", marginBottom: "10px" }}>📋 Hướng dẫn setup (1 lần duy nhất):</div>
        ...
        <div style={{ marginBottom: "14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
            ...
            <button onClick={copy} style={{ background: copied ? "#e6f8ee" : "#f4f8fc", border: `1px solid ${copied ? "#80d0a8" : "#c8ddf0"}`, borderRadius: "6px", padding: "3px 10px", color: copied ? "#1a7a45" : "#1a6fba", fontSize: "11px", cursor: "pointer", fontFamily: "inherit" }}>
        ...
        <Field label="Web App URL (dán vào đây sau khi deploy)">
